...
- **Règles** : un `PipelineBuilder` typé enchaîne les règles de transition (`Initial` → `Attacked` → `Hit` → `Wounded` → `Saved` → `Damaged`), avec vérification à la compilation de l’ordre correct.

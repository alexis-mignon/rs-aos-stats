...
impl From<i32> for CharacteristicPy {
    fn from(val: i32) -> Self {
        CharacteristicPy {characteristic: Characteristic::Value(val as u32)}
    }
}
...
fn extract_characteristic(value: &Bound<'_, PyAny>) -> PyResult<Characteristic> {
    ...
    else if let Ok(dice_str) = value.extract::<String>() {Ok(Characteristic::DiceRoll(
        DiceRoll::from_str(&dice_str).map_err(|e| PyValueError::new_err(e.to_string()))?
    ))}
    else if let Ok(char_roll) = TryInto::try_into(value) {Ok(Characteristic::DiceRoll(char_roll))}
    else if let Ok(charac) = value.extract::<CharacteristicPy>() {Ok(charac.into())}
    else {Err(PyValueError::new_err("Could not convert to Characteristic"))}
}

#[pymethods]
impl AttackStatsPy {
    #[new]
    fn new(attacks: Bound<'_, PyAny>, to_hit: i32, to_wound: i32, rend: i32, damages: Bound<'_, PyAny>) -> PyResult<Self> {
        Ok(AttackStatsPy {
